...
root.innerHTML=`<a class="svk40-back" href="news.html">← Alle Meldungen</a><div class="svk40-article-meta"><span>${S.esc(n.category||"SVK")}</span><time>${S.date(n.date||n.published_at)}</time></div><h1>${S.esc(n.title)}</h1>${n.teaser?`<p class="svk40-lead">${S.esc(n.teaser)}</p>`:""}${n.image_url?`<img class="svk40-article-image" src="${S.esc(n.image_url)}" alt="${S.esc(n.title)}">`:""}<div class="svk40-copy"><p>${paras}</p></div>`}
